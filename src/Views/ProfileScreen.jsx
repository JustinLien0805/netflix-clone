import React from "react";
import NavBar from "../components/NavBar";
import Plans from "../components/Plans";
import { useSelector } from "react-redux";
import { selectUser } from "../features/userSlice";
import { auth } from "../firebase";
const ProfileScreen = () => {
  const user = useSelector(selectUser);
  return (
    <div className="flex justify-center">
      <NavBar />
      <div className="h-[100%-80px] w-2/3 text-white flex flex-col mt-20 p-20 justify-center items-center">
        <h1 className="text-5xl font-bold mb-4 w-full text-left border-b-2 border-b-[#1c1c1c]">Profile</h1>
        <div className="flex justify-center items-start w-full space-x-4 my-4">
          <img
            src="data:image/png;base64,"
            alt=""
            className="h-20 w-20"
          />
          <div className="w-full">
            <h2 className="text-3xl font-bold">Member: </h2>
            <h2 className="text-2xl font-bold mb-8">{user.email}</h2>
            <h2 className="text-3xl font-bold border-b-2 border-b-[#1c1c1c]">Plans</h2>
            <Plans></Plans>
          </div>
        </div>

        <button
          onClick={() => {
            auth.signOut();
          }}
          className="w-full bg-red-600 rounded-md px-4 py-2 font-bold"
        >
          Sign Up
        </button>
      </div>
    </div>
  );
};

export default ProfileScreen;
